...
def create_voting_dict():
    ...
    """
    result = {}
    for person in voting_data:
        s = person.split()
        result[s[0]] = []
        for i in range(3, len(s)):
            result[s[0]].append(int(s[i]))
    return result
...
def most_similar(sen, voting_dict):
    ...
    result = ""
    largest = 0
    for person in voting_dict:
        if person == sen: continue
        tmp = policy_compare(sen, person, voting_dict)
        if tmp > largest:
            largest = tmp
            result = person
...
def bitter_rivals(voting_dict):
    ...
    for i in range(size):
        for j in range(i+1, size):
            similarity = policy_compare(sens[i], sens[j], voting_dict)
            if smallest == None:
                smallest = similarity
                result = (sens[i], sens[j])
            elif similarity < smallest:
                smallest = similarity
                result = (sens[i], sens[j])
    return result
